// Sample Data
var channels = [
    { id: 101, number: '01', title: 'HBO HD', description: 'Live Sports & Premier Movies' },
    { id: 102, number: '02', title: 'Discovery', description: 'Documentaries & Nature' },
    { id: 103, number: '03', title: 'CNN', description: '24/7 Global News Coverage' },
    { id: 104, number: '04', title: 'Eurosport', description: 'Live Football & Tennis' }
];

// --- 1. Template Factory Function ---
function createChannelCard(data) {
    var template = document.getElementById('channel-card-template');
    var clone = document.importNode(template.content, true);

    var card = clone.querySelector('.channel-card');
    var numberEl = clone.querySelector('.card-number');
    var titleEl = clone.querySelector('.card-title');
    var descEl = clone.querySelector('.card-description');
...
function renderChannelGrid(items) {
    var container = document.getElementById('channel-grid');
...
function handleChannelSelect(targetCard) {
    var channelId = targetCard.getAttribute('data-id');
    var channelTitle = targetCard.querySelector('.card-title').textContent;

    console.log('Action triggered on Channel ID:', channelId, 'Title:', channelTitle);

    // Example UI response: visual active feedback
    var currentActive = document.querySelector('.channel-card.active');
    if (currentActive) {
...
function initEventDelegation() {
    var gridContainer = document.getElementById('channel-grid');

    // A. Click Event Listener (Handles Mouse / Remote Pointer)
    gridContainer.addEventListener('click', function (event) {
        var card = event.target.closest('.channel-card');
        if (card) {
